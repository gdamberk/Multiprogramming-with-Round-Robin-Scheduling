...
Thread::Fork(VoidFunctionPtr func, void *arg)
{
    ...
    scheduler->ReadyToRunT(this);	// ReadyToRun assumes that interrupts 
					// are disabled!
...
Thread::Begin ()
{
    ...
    kernel->currentProcess->threadScheduler->CheckToBeDestroyed();
...
//----------------------------------------------------------------------
    ...
    Thread::Yield ()
    {
        ...
        nextThread = kernel->currentProcess->threadScheduler->FindNextToRunT();
        if (nextThread != NULL) {
    	   nextThread = kernel->currentProcess->threadScheduler->FindNextToRunT();
           if(nextThread != NULL){
    	        kernel->currentProcess->threadScheduler->RunT(nextThread, FALSE);
...
Thread::Sleep (bool finishing)
{
    ...
    status = BLOCKED;
    while ((nextThread = kernel->currentProcess->threadScheduler->FindNextToRunT()) == NULL)
	kernel->interrupt->Idle();	// no one to run, wait for an interrupt
    cout<< "After while thread: " << name << endl;
    // returns when it's time for us to run
    kernel->currentProcess->threadScheduler->RunT(nextThread, finishing); 
}
